...
import { Button, Select, Space, Tag, Tooltip, Modal, message } from 'antd';
import {
    DeleteOutlined,
    PlusOutlined,
    SaveOutlined,
...
import { SEVERITY_COLORS, type BboxAnnotation } from '../../utils/bboxOverlay';
...
export default function BboxEditor({
    ...
    const getResizeHandle = (x: number, y: number, box: EditableBox): ResizeHandle => {
        const [bx, by, bw, bh] = box.bbox;
        ...
        <div className="bbox-editor">
            <div className="bbox-editor-toolbar">
                <Space>
                    ...
                    <Button icon={<UndoOutlined />} onClick={handleUndo} disabled={history.length <= 1}>
                        Undo
                    </Button>
                    <Button icon={<ZoomInOutlined />} onClick={() => setScale((s) => Math.min(2, s + 0.1))}>
                        Zoom In
                    </Button>
                    <Button icon={<ZoomOutOutlined />} onClick={() => setScale((s) => Math.max(0.3, s - 0.1))}>
                        Zoom Out
                    </Button>
                    <span style={{ marginLeft: 16 }}>Scale: {Math.round(scale * 100)}%</span>
                </Space>
                <Space>
                ...
                <div className="bbox-editor-properties">
                    <Space wrap>
                        ...
                        <Select
                            ...
                            onChange={(value) =>
                                setBoxes((prev) =>
                                    prev.map((b) => (b.id === selectedBoxId ? { ...b, label: value } : b))
                                )
                            }
                        >
                        ...
                        <Select
                            ...
                            onChange={(value) =>
                                setBoxes((prev) =>
                                    prev.map((b) => (b.id === selectedBoxId ? { ...b, severity: value } : b))
                                )
                            }
                        >
            ...
            <div className="bbox-editor-canvas-container" ref={containerRef}>
                <canvas
                    ...
                    style={{ cursor: isDrawing ? 'crosshair' : isDragging ? 'move' : 'default' }}
                />
            </div>
            ...
            <div className="bbox-editor-box-list">
                ...
                {boxes.map((box, idx) => (
                    ...
                    >
                        <Tag color={SEVERITY_COLORS[box.severity]?.strokeColor}>{idx + 1}</Tag>
                        <span>{box.label}</span>
