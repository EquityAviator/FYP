    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            annotations.forEach((annotation, index) => {
                ...
                const style = SEVERITY_COLORS[annotation.severity] || SEVERITY_COLORS.medium;
...
export function validateBbox(
    ...
    imageWidth: number,
    imageHeight: number
): { valid: boolean; adjusted: [number, number, number, number]; warnings: string[] } {
    ...
    if (x + width > imageWidth) {
        ...
        warnings.push(`width (${width}) exceeded image bounds, adjusted to ${newWidth}`);
        width = newWidth;
    }
    ...
    if (y + height > imageHeight) {
        ...
        warnings.push(`height (${height}) exceeded image bounds, adjusted to ${newHeight}`);
        height = newHeight;
    }

    return {
        valid: warnings.length === 0,
        adjusted: [Math.round(x), Math.round(y), Math.round(width), Math.round(height)],
        warnings,
    };
}
