...
import { getDatasetEntries } from './datasetDB';
...
export const DARK_PATTERN_CATEGORIES: COCOCategory[] = [
    ...
    { id: 2, name: 'Dead End/Roach Motel', supercategory: 'dark_pattern' },
    { id: 3, name: 'Price Comparison Prevention', supercategory: 'dark_pattern' },
    { id: 4, name: 'Disguised Ad / Bait & Switch', supercategory: 'dark_pattern' },
    { id: 5, name: 'Reference Pricing', supercategory: 'dark_pattern' },
    { id: 6, name: 'False Hierarchy', supercategory: 'dark_pattern' },
    { id: 7, name: 'Bundling / Auto-add / Bad Defaults', supercategory: 'dark_pattern' },
    { id: 8, name: 'Pressured Selling / FOMO / Urgency', supercategory: 'dark_pattern' },
    { id: 9, name: 'Scarcity & Popularity', supercategory: 'dark_pattern' },
    { id: 10, name: 'Hard To Close', supercategory: 'dark_pattern' },
    { id: 11, name: 'Trick Questions / Confirmshaming', supercategory: 'dark_pattern' },
    { id: 12, name: 'Hidden Information', supercategory: 'dark_pattern' },
    { id: 13, name: 'Infinite Scrolling', supercategory: 'dark_pattern' },
    { id: 14, name: 'Forced Ads / Autoplay', supercategory: 'dark_pattern' },
];

function getCategoryId(patternType: string): number {
    const category = DARK_PATTERN_CATEGORIES.find(
        c => c.name.toLowerCase() === patternType.toLowerCase() ||
            patternType.toLowerCase().includes(c.name.toLowerCase().split('/')[0].trim())
    );
...
export async function exportAsCOCO(): Promise<Blob> {
    ...
    zip.file('annotations/instances_train.json', JSON.stringify(cocoDataset, null, 2));
...
export async function exportAnnotatedImages(): Promise<Blob> {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (entry.patterns.length > 0 && annotatedFolder) {
            try {
                const annotations = entry.patterns
                ...
                const annotatedImage = await drawBboxesOnImage(entry.screenshot, annotations);
                const annotatedMatch = annotatedImage.match(/^data:(.*?);base64,(.*)$/);
                if (annotatedMatch) {
                    annotatedFolder.file(`${baseName}_annotated.png`, annotatedMatch[2], { base64: true });
                }
            } catch (error) {
    ...
    const manifest = entries.map((e, i) => ({
        id: e.id,
        url: e.url,
        pattern_count: e.patterns.length,
